...
class SVMClassifier:
    def __init__(self, train_data, train_labels, best_words, C):
        ...
        self.file = pd.read_csv(train_data, encoding = 'utf-8')
        self.comment = train_labels
        self.thres = C
    ...
    def load_model(self):
        self.nowlist = []
        for i in range(3):
            self.nowlist.append(self.file.iloc[self.comment,i+1])
        self.model = AipNlp(self.nowlist[0],self.nowlist[1], self.nowlist[2])
    def classify(self, data):
        result = self.model.sentimentClassify(data)
        score = result[self.thres][0][self.best_words]
        if(score>0.5):
            return 1
        else:
            return 0